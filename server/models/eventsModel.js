...
async function postEvent(eventDetails) {
  try {
    ...
    console.log('eventDetails.auditoriumName', eventDetails.auditoriumName);
   
    const auditoriumQuery =
      `SELECT auditoriumId FROM auditoriums WHERE auditoriumName = ?`;
    const auditoriumResult = await pool.query(auditoriumQuery, [eventDetails.auditoriumName]);
